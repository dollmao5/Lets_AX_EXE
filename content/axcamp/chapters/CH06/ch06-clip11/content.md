...
**팀장 활용 시나리오**: 임원 보고용 대시보드·팀 공유용 현황판을 AI로 만들 때, 스타일을 말로 지정하지 못하면 매번 다른 결과가 나옵니다 — 이 코드북은 그 지정을 표준화합니다.
...
백색 기반, 얇은 선, 정렬 중심. 임원 보고용 대시보드와 리서치 화면에 가장 무난합니다.
...
대체로 절제되어 있지만, 한두 개의 포인트 컬러와 CTA로 핵심 메시지를 밀어줍니다.
...
두꺼운 보더, 큰 대비, 직설적인 CTA. 짧고 강한 메시지나 교육용 데모 앱에 가장 잘 맞습니다.
...
광고처럼 큰 메시지와 강한 포인트 컬러를 전면에 둡니다. 제품 데모나 이벤트용 앱에 적합합니다.
...
보통은 **톤 패밀리만 말해도 색감 방향**이 같이 결정됩니다. 브랜드 컬러를 꼭 써야 하거나, 포인트 색을 제한하고 싶거나, 다크 네이비처럼 특정 무드를 원할 때만 색상을 추가로 지정하면 됩니다.
