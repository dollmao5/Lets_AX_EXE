...
## 3-B. 개인별 2차 캔버스 (팀 합의 + 개인 원문 병합 — 다음 실습의 개인 자료)

각 Round 요약 생성 직후, 보드의 **[2차 캔버스 생성]** 을 누르면 제출자 전원에게
"① 나의 결론(개인 작성 원문·우선) ② 팀 공통 합의(참고) ③ 팀 내 관점 차이" 구조의
개인별 2차 캔버스가 만들어집니다 (AI 호출 없는 즉시 병합 — 비용·대기 없음, 재실행 안전).

| 항목 | 내용 |
|---|---|
| 실행 시점 | **[요약 생성] 완료 후** [2차 캔버스 생성] 클릭 (요약 전에 눌러도 개인 원문만으로 생성됨 — 요약 후 다시 누르면 팀 합의 반영) |
| 교육생 활용 1 | **Round 2·3 캔버스 상단 "이전 Round 2차 캔버스 불러오기"** — 조+이름 입력 → 직전 라운드 결과를 참고하며 토론·ChatGPT 실습(복사 버튼) |
| 교육생 활용 2 | **CH04 실습 페이지 / Round 3 제출 뒤 "통합본 다운로드"** — `CH04_R1-3_팀토론2차캔버스_성명.md` 를 받아 NotebookLM '필수 3' 소스로 업로드 (팀 공용 파일 대신 개인화 파일) |
...
- **데이터 위치**: `data/wrapup/{차수}/{round}/` (개인별 2차 캔버스는 그 아래 `canvas2/`) — `data/`는 gitignore라 공개 저장소·Pages에 절대 올라가지 않음. 원격 백업은 프라이빗 저장소 `dollmao5/Lets_AX_Wrapup_DATA` (동일 레이아웃이라 pull/push 동기화에 canvas2도 포함됨)
